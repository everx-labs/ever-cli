...
contract EncryptionBoxInputDebot is Debot {
    ...
    function encrypt() public {
        m_data = "Hello world!";
        Sdk.encrypt(tvm.functionId(setEncrypted), m_handle, m_data);
    }

    function setEncrypted(uint32 result, bytes encrypted) public {
        Sdk.decrypt(tvm.functionId(setDecrypted), m_handle, encrypted);
    }

    function setDecrypted(uint32 result, bytes decrypted) public {
        require(decrypted == m_data, 111);
    }
