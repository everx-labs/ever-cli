...
fn _run_command_and_decode_json(command: &str, local_node: bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let out = cmd.args(command.split(' ').collect::<Vec<&str>>())
        .output()
...
fn test_json_output_2() -> Result<(), Box<dyn std::error::Error>> {
    ...
    run_command_and_decode_json(&format!("fee deploy --abi {} --sign {} {} {{}}", DEPOOL_ABI, key_path, DEPOOL_TVC))?;
    run_command_and_decode_json(&format!("deploy --abi {} --sign {} {} {{}}", DEPOOL_ABI, key_path, DEPOOL_TVC))?;
...
fn test_json_output_5() -> Result<(), Box<dyn std::error::Error>> {
    ...
    run_command_and_decode_json(&format!("deployx --abi tests/samples/fakeDepool.abi.json --keys {} tests/samples/fakeDepool.tvc ", GIVER_V2_KEY))?;
    ...
    run_command_and_decode_json(&format!("genaddr tests/account.boc --abi tests/samples/wallet.abi.json --setkey {}", GIVER_V2_KEY))?;
