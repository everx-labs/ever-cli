...
use ton_client_rs::{EncodedMessage, OrderBy, SortDirection, TonAddress, TonClient};
use ton_sdk;
...
impl TxnInfo {
    pub fn from_json(txn: &serde_json::Value) -> TxnInfo {
        ...
        let tvm_exit_code = txn["compute"]["exit_code"].as_i64().unwrap();
        let tvm_status = txn["compute"]["success"].as_bool().unwrap();
        TxnInfo {
            id,
            aborted,
            now,
...
fn query_transactions(
    ...
    let txns = ton.queries.transactions.query(
        filter.into(),
...
fn query_all_account_transactions(
    ...
    let txns = query_transactions(conf, query)?;
