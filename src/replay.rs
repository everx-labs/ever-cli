...
use ton_block::{
    Account, Block, ConfigParams, Deserializable, HashmapAugType, Message, Serializable,
...
use ton_types::{serialize_tree_of_cells, BuilderData, SliceData, UInt256};
...
) -> Result<BlockchainConfig, failure::Error> {
    let config_cell = config_account
        .get_data()
        .ok_or(err_msg("Failed to get account's data"))?
        .reference(0)
        .ok();
    let config_params =
...
pub async fn fetch_block(
    ...
    let wid = block.result.get(0).unwrap()["workchain_id"]
        .as_i64()
        .unwrap();
    let end_lt = block.result.get(0).unwrap()["end_lt"]
        .as_str()
        .unwrap()
